...
from layers.voxel_mix import voxel_mix_net
__all__ = ['LSSFPN']
...
class LSSFPN(nn.Module):
    def __init__(self, x_bound, y_bound, z_bound, d_bound, final_dim,
        ...
        # TODO hard code here, only for test!!!!
        self.z_bounds = [[-6, -3, 3],[-3, -2, 2],[-2, -1, 1],[-1, 0, 1],[0, 2, 2],[2, 4, 2],[-6, 4, 10],[-5, 3, 8],[-4, 2, 6]]
        self.register_buffer(
            f'voxel_size',
            torch.Tensor([[row[2] for row in [x_bound, y_bound, zb]] for zb in self.z_bounds])
                         )
        self.register_buffer(
            f'voxel_coord',
            torch.Tensor([[row[0] + row[2] / 2.0 for row in [x_bound, y_bound, zb]] for zb in self.z_bounds]))
        self.register_buffer(
            f'voxel_num',
            torch.LongTensor([[(row[1] - row[0]) / row[2] for row in [x_bound, y_bound, zb]]for zb in self.z_bounds]))
        self.voxel_mix_net = voxel_mix_net()
    ...
    def _forward_single_sweep(self,
        ...
        # TODO hard code here!!!! careful!!
        feature_maps = []
        for i in range(len(self.z_bounds)):
            tmp_geom_xyz = ((geom_xyz - (self.voxel_coord[i] - self.voxel_size[i] / 2.0)) /
                        self.voxel_size[i]).int()
            feature_maps.append(voxel_pooling(tmp_geom_xyz, img_feat_with_depth.contiguous(),
                                        self.voxel_num[i].cuda()))
        overlap_feature = torch.cat(feature_maps,dim=1)
        feature_map = self.voxel_mix_net(overlap_feature)
        feature_map = feature_map.type(torch.cuda.FloatTensor)
        if is_return_depth:
    ...
    def forward(self,
        ...
        if is_return_depth:
            return torch.cat(ret_feature_list, 1), key_frame_res[1], key_frame_res[2]
        else:
